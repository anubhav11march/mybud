...
exports.takeChallenge = async (req, res) => {
	try {
		...
		days = parseInt(days);
		let user_challenge = await Challenge.findOne({
			name: challenge,
			userid: user,
			isCompleted: false,
		}); //checking if user has already taken part in this challenge and not yet finished
		if (user_challenge) {
			return res
				.status(400)
				.json(
					errormessage(
						'You have already taken this challenge and not yet finished it!'
					)
				);
		}
		...
		if (ismatch.length >= 3) {
			return res
				.status(400)
				.json(errormessage('Already Enrolled in 3 Challenges!'));
		}
		...
		res
			.status(200)
			.json(
				successmessage('Successfuly enrolled in the Challenge!', user_challenge)
			);
	} catch (err) {
...
exports.dailyattendence = async (req, res) => {
	try {
		...
		user = mongoose.Types.ObjectId(JSON.parse(user));
		let isMatch = await Challenge.findOne({
			name: challenge,
			userid: user,
			isCompleted: true,
		});
		if (isMatch) {
			return res
				.status(400)
				.json(errormessage('You have already completed the challenge!'));
		}
		isMatch = await Challenge.findOne({
			name: challenge,
			userid: user,
			counter: { $in: [todayDate()] },
		});
		if (isMatch) {
			return res
				.status(400)
				.json(
					errormessage('You have already marked the attendence for today!')
				);
		}
		...
		let updatedchallenge = await Challenge.findOneAndUpdate(
			{ name: challenge, userid: user },
			updates,
			{ new: true }
		);
		if (!updatedchallenge) {
		...
		res
			.status(200)
			.json(successmessage('Successfuly Updated!', updatedchallenge));
	} catch (err) {
		res.status(400).json(errormessage(err.message));
	}
};
...
exports.getAttendence = async (req, res) => {
	try {
		...
		if (!challenge) {
			return res
				.status(200)
				.json(successmessage('No data found with this user'));
		}
		res
			.status(200)
			.json(successmessage('Challenge Attendence', challenge.counter));
	} catch (err) {
...
exports.deleteChallenge = async (req, res) => {
	try {
		...
		let deletedchallenge = await Challenge.findOneAndDelete({
			_id: challengeid,
		});
		if (!deletedchallenge) {
		...
		res
			.status(200)
			.json(successmessage('Successfuly Deleted!', deletedchallenge));
	} catch (err) {
		res.status(400).json(errormessage(err.message));
	}
};
